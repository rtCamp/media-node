...
class Job(db.Model):
    ...
    request_format = db.Column(db.Enum('mp4', 'mp3', 'thumbnails',name = 'request_format_type'),default='mp4')

    # queued, completed, processing, error
    status = db.Column(db.Enum('queued', 'completed', 'processing', 'error'),default='queued')
    bandwidth = db.Column(db.Integer)
    thumb_count = db.Column(db.Integer)
    callback_url = db.Column(db.String(255))
    duration = db.Column(db.Float)

    def __init__(self, api_job_id , file_path, file_url, request_format, api_key_id = 0, status = "queued", bandwidth = 0, thumb_count = 0 , callback_url = "", duration = 0):
